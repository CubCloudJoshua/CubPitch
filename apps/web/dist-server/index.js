...
import { deckId, isSafeId, parseDeck, reviewDeck, starterDeck } from '@cubpitch/core';
...
route('POST', '/api/decks', async (request, response) => {
    ...
    if (!body.company)
        return send(response, 400, { error: 'company is required' });
...
route('PUT', '/api/decks/:id', async (request, response, params) => {
    ...
    if (deck.id !== params['id'])
        return send(response, 400, { error: 'Deck id does not match the URL' });
    try {
...
route('DELETE', '/api/decks/:id', async (_request, response, params) => {
    try {
        ...
        if (error instanceof DeckNotFoundError)
            return send(response, 404, { error: error.message });
        throw error;
    }
});
...
const server = createServer((request, response) => {
    void handle(request, response).catch((error) => {
        process.stderr.write(`${request.method} ${request.url} failed: ${error.stack}\n`);
...
function serveStatic(pathname, response) {
    ...
    if (!candidate.startsWith(STATIC_DIR))
        return void send(response, 403, { error: 'Forbidden' });
